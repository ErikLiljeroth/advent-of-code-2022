...
class Monkey_PartII:
    ...
    def _update_worry_level(self, old_worry_level, divisor_product) -> int:
        if self.operation[0] == '+':
            if self.operation[1] != 'old':
                addend = int(self.operation[1])
                result = old_worry_level + addend
                return result % divisor_product
            else:
                result = old_worry_level + old_worry_level
                return result % divisor_product
        elif self.operation[0] == '*':
            if self.operation[1] != 'old':
                factor = int(self.operation[1])
                result = old_worry_level * factor
                return result % divisor_product
            else:
                result = old_worry_level * old_worry_level
                return result % divisor_product
    ...
    def inspect_and_throw(self, divisor_product) -> dict:
        throw_items = {}
        ...
        for item in self.items:
            ...
            new_worry_level = self._update_worry_level(item, divisor_product)
...
class MonkeySimulation_PartII:

    def __init__(self) -> None:
        self.monkeys = {}
        self.M = 0

    def add_monkey(self, monkey:Monkey_PartII):
        self.monkeys[monkey.monkey_nbr] = monkey

    def set_divisor_product(self):
        divisors = []
        for monkey_nbr in self.monkeys:
            divisors.append(self.monkeys[monkey_nbr].divisible)
        self.M = divisors[0]
    ...
    def iterate_round(self):
        for monkey_nbr in self.monkeys:
            throw_items = self.monkeys[monkey_nbr].inspect_and_throw(self.M)
            #print(throw_items)
...
def main():
    ...
    m_sim = MonkeySimulation_PartII()

    monkeys = parse_input(input)
    for nbr in monkeys:
        m_sim.add_monkey(Monkey_PartII(nbr, monkeys[nbr][0], monkeys[nbr][1], monkeys[nbr][2], monkeys[nbr][3], monkeys[nbr][4]))
    m_sim.set_divisor_product()
    ...
    print('Answer part II:',monkey_business)
